const ACTIONS=['start','pause','restart','move','swing','draw','guard','parry','dodge','standoff','sound','look','stance','continue'];
export function registerGameTools(game) {
  ...
  register({name:'temple_read_state',description:'Read the current Temple Night run: phase (title, roam, arrival, standoff, fight, clear, dead, victory), wave, health, guard, parry window, dodge, stance, the enemies with their health and distance, the standoff and banner, the result, the player position and speed, sound and readiness.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:()=>game.getState()});
