export function registerGameTools(game) {
  ...
  register({name:'temple_read_state',description:'Read the current Temple Night player position, movement speed, sword stance, body and animation readiness and pause state.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:()=>game.getState()});
  register({name:'temple_control',description:'Play, pause, move, swing the sword, or choose a stance (stone, water, wind, moon) in the current Temple Night scene using the same controls as the player.',inputSchema:{type:'object',properties:{action:{enum:['start','pause','move','swing','draw','sound','look','stance']},degrees:{type:'number',minimum:-180,maximum:180},direction:{enum:['forward','back','left','right']},seconds:{type:'number',minimum:.05,maximum:3},run:{type:'boolean'},stance:{enum:['stone','water','wind','moon']}},required:['action'],additionalProperties:false},execute:async input=>{
    if(!input || !['start','pause','move','swing','draw','sound','look','stance'].includes(input.action))throw new Error('Unknown action');
