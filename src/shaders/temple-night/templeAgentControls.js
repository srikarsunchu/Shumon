const ACTIONS=['start','pause','restart','move','swing','draw','guard','parry','dodge','standoff','sound','look','stance','continue','interact'];
export function registerGameTools(game) {
  ...
  register({name:'temple_read_state',description:'Read the current Temple Night run: phase (title, roam, arrival, standoff, fight, clear, dead, victory, ceremony, ending, complete), story beat and interaction availability, wave, health, guard, parry window, dodge, stance, the enemies with their health and distance, the standoff and banner, the result, the player position and speed, sound and readiness.',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:()=>game.getState()});
  register({name:'temple_control',description:'Play the Temple Night duel with the same controls as the player: start, pause or restart the run, move, swing the sword, draw, guard (down/up), parry, dodge, hold and release the standoff (down/up), look, choose a stance (stone, water, wind, moon) take the displayed sword (interact), continue or toggle sound.',inputSchema:{type:'object',properties:{action:{enum:ACTIONS},degrees:{type:'number',minimum:-180,maximum:180},direction:{enum:['forward','back','left','right']},seconds:{type:'number',minimum:.05,maximum:3},run:{type:'boolean'},down:{type:'boolean',description:'guard and standoff: press (true, the default) or release (false)'},stance:{enum:['stone','water','wind','moon']}},required:['action'],additionalProperties:false},execute:async input=>{
    ...
    else if(input.action==='interact')game.interact();
    else if(input.action==='continue')game.continueExploring();
